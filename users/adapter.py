...
from allauth.compat import authenticate, is_authenticated, reverse, validate_password
...
class AccountAdapter(object):
    ...
    def get_login_redirect_url(self, request):
        ...
        if url:
            warnings.warn("LOGIN_REDIRECT_URLNAME is deprecated, simply"
                          ...
                          DeprecationWarning)
        elif request.user.usertype:
            url = settings.LOGIN_REDIRECT_URL_LIST[request.user.usertype.name]
        else:
    ...
    def save_user(self, request, user, form, commit=True):
        ...
        usertype = Group.objects.get(name=usertype)
    ...
    def clean_first_name(self, first_name):
        """
        Validates the first_name. You can hook into this if you want to
        (dynamically) restrict what first_name can be chosen.
        """
        # check for special characters
        forbiden_char = {'~', ':', "'", '+', '[', '\\', '@', '^', '{', '%', '(', '-', '"', '*', '|', ',', '&', '<', '`', '}', '.', '_', '=', ']', '!', '>', ';', '?', '#', '$', ')', '/'}
        for char in forbiden_char:
            if char in first_name:
                raise forms.ValidationError("Special Characters are not allowed in the First name")
                break
        # check name size
        if len(first_name) <= 1:
            forms.ValidationError("First Name is too short")
        elif len(first_name) >= 50:
            forms.ValidationError("First Name is too long")

        return first_name

    def clean_last_name(self, last_name):
        """
        Validates the last_name. You can hook into this if you want to
        (dynamically) restrict what last_name can be chosen.
        """
        # check for special characters
        forbiden_char = {'~', ':', "'", '+', '[', '\\', '@', '^', '{', '%', '(', '-', '"', '*', '|', ',', '&', '<', '`', '}', '.', '_', '=', ']', '!', '>', ';', '?', '#', '$', ')', '/'}
        for char in forbiden_char:
            if char in last_name:
                raise forms.ValidationError("Special Characters are not allowed in the Last name")
                break
        # check name size
        if len(last_name) <= 1:
            forms.ValidationError("Last Name is too short")
        elif len(last_name) >= 100:
            forms.ValidationError("Last Name is too long")
        return last_name
    ...
    def clean_usertype(self, usertype, user=None):
        """
        Validates a usertype, to check that it's either CLIENT or HUB.
        """
        if not (usertype == "HUB" or usertype=="CLIENT"):
            raise forms.ValidationError("User Type must be either 'HUB' or 'CLIENT'")
        return usertype
